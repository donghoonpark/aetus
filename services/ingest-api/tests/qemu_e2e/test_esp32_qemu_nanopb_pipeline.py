...
def _idf_command(command: str, *, timeout: float) -> subprocess.CompletedProcess[str]:
    idf_path = _idf_path()
...
def _capture_qemu_payload_hex(timeout: float = 120.0) -> str:
    ...
    try:
        while time.time() < deadline:
            ...
            while "\n" in partial:
                ...
                if stripped == "AETUS_PROTO_HEX_BEGIN":
                    in_payload = True
                    hex_lines = []
                    continue
                if stripped == "AETUS_PROTO_HEX_END" and in_payload:
    ...
    output_tail = "\n".join(lines[-80:])
    raise RuntimeError(f"Timed out waiting for QEMU protobuf stream:\n{output_tail}")
